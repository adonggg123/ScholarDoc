...
class MLService {
  ...
  Future<Map<String, dynamic>> validateDocumentClarity(String filePath) async {
    ...
    return {
      ...
      'clarityScore': isBlurry ? _random.nextDouble() * 40 : 60 + _random.nextDouble() * 40,
  ...
  Future<String> classifyDocument(String filePath) async {
    ...
    final types = ['ID Card', 'Billing Statement', 'Enrollment Form', 'Scholarship Agreement'];
  ...
  Map<String, dynamic> detectSASuspiciousPattern(String saNumber) {
    ...
    final sequentialDigits = RegExp(r'1234|2345|3456|4567|5678|6789|9876|8765|7654');
    ...
    } else if (repeatingDigits.hasMatch(saNumber) || sequentialDigits.hasMatch(saNumber)) {
      return {
        'isSuspicious': true,
        'confidence': 85.0,
  ...
  Future<bool> detectDuplicateSubmission(String fileHash, String studentId) async {
  ...
  double predictSubmissionRisk(Map<String, dynamic> studentData) {
    ...
    if (studentData['familyDetails'] == null || studentData['familyDetails'].isEmpty) {
      riskScore += 30.0;
    }
